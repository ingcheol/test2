  ...
  let ai6 = {
    ...
    send: async function(pngBlob){
      ...
      try {
        ...
        // ===== 대시보드로 AI 메시지 전송 =====
        this.sendToDashboard(pngBlob, answerJson.text);
    ...
    // 대시보드로 메시지 전송하는 새로운 함수
    sendToDashboard: async function(pngBlob, aiText){
      try {
        const formData = new FormData();
        formData.append('attach', pngBlob, 'frame.png');

        // AI 텍스트를 짧게 요약해서 전송 (대시보드 표시용)
        const summaryText = aiText.length > 100
        ...
        if(response.ok){
          console.log('대시보드로 메시지 전송 성공');
        }else{
          console.log('대시보드로 메시지 전송 실패');
        }
      } catch(error) {
        console.error('대시보드 전송 오류:', error);
      }
    },
